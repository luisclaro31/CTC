...
class controladorDepartamento extends CI_Controller
{
   ...
   {  
       ...
       if($this->session->userdata('perfil') != "Administracion")
       {
        ...
        $parametros = $this->ObtenerListaValoresDepartamento($ordenamiento, $desde, $codigo);        
        
        $opciones['per_page'] = 10;
        $opciones['base_url'] = '/index.php/controladorDepartamento/index/'.$ordenamiento.'/';
       ...
       $this->load->view('vistaDepartamento', $parametros);
   }
      
   public function AdicionarDepartamento()
   {
        $codigo = $_POST['txtCodigoDepartamentoAdic'];
        $datosAuditoria = $this->Auditoria($codigo, "AD");              
        $rpta = $this->procedimientos_model->SetProcedure("departamento_insertar",
                "'".$codigo."',
                '".$_POST['txtNombreDepartamentoAdic']."'                
                ", $datosAuditoria);

       $parametros = $this->ObtenerListaValoresDepartamento(1, 0, "");
   ...
   {   
       $codigo = $_POST['txtCodigoDepartamento'];
       $datosAuditoria = $this->Auditoria($codigo, "MD");                          
       $rpta = $this->procedimientos_model->SetProcedure("departamento_modificar",
                "'".$codigo."',
                '".$_POST['txtNombreDepartamento']."'
                ", $datosAuditoria);
    ...
    */
   public function EliminarDepartamento($codigo)
   {
       $datosAuditoria = $this->Auditoria($codigo, "EL");
        $rpta = $this->procedimientos_model->SetProcedure("departamento_eliminar","'$codigo'", $datosAuditoria);        
    ...
    {
        $this->ObtenerListaValoresDepartamento();                
        $opciones = array();
        $desde = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        
        $opciones['per_page'] = 10;
        $opciones['base_url'] = '/index.php/controladorDepartamento/index/1/';
        $opciones['total_rows'] = $parametros['conteoTotal'][0]['total'];
        $opciones['uri_segment'] = 4;
    ...
    */
   public function ConsultarDepartamento($codigo, $consultar = "")
   {
        $registro = $this->procedimientos_model->GetProcedure("departamento_seleccionar_por_codigo","'$codigo'");
        $parametros = $this->ObtenerListaValoresDepartamento();
   ...
   public function GenerarExcel()
   {
        ...
        */
        ...
        // Se renombra la hoja
        $objSheet->setTitle('DatosDepartamento');

        //Se obtiene la data del reporte
        $registrosConsultaDepartamento = $this->procedimientos_model->GetProcedure("departamento_seleccionar","");
        $i = 2;
        
        //Titulos
        $objSheet->getCell('A1')->setValue(utf8_encode('CODIGO'));
        $objSheet->getCell('B1')->setValue(utf8_encode('NOMBRE DEPARTAMENTO'));                
            
        foreach($registrosConsultaDepartamento as $registro)        
        {   
            //Valores
        ...
        $objWriter->save('temp/Departamentos.xlsx');               
        header('Location: /temp/Departamentos.xlsx');
   ...
   public function GenerarPdf()
   {
        ...
        $pdf->Cabecera('LISTADO DE DEPARTAMENTOS', '', 125);        
        ...
        $header = array('CODIGO', 'NOMBRE DEPARTAMENTO');
        $pdf->Titulo($header,$w);
        //Se obtiene la data del reporte
        $registrosConsultaDepartamentos = $this->procedimientos_model->GetProcedure("departamento_seleccionar","");
        
        foreach($registrosConsultaDepartamentos as $registro)
        {               
   ...
   {       
       $existe = $this->procedimientos_model->GetProcedure("departamento_validar_codigo","'$codigo'");          
       if($existe[0]['count'] > 0)
        echo 'El Codigo de departamento ya existe, escriba uno nuevo.';       
   }
   
   /*
    * Listas de valores básicas
    */
   private function ObtenerListaValoresDepartamento($ordenamiento = 1, $pagina = 1, $codigo = "")
   {       
       $registrosConsultaDepartamentos = $this->procedimientos_model->GetProcedure("departamento_seleccionar_principal","'".$ordenamiento."', '".$pagina."', '".$codigo."'");
       $nombreDepartamento = $this->procedimientos_model->GetProcedure("departamento_seleccionar","");       
       $conteoTotal = $this->procedimientos_model->GetProcedure("departamento_seleccionar_conteo_total","");
       $usuario = array('perfil' => $this->session->userdata('perfil'),
                        'idUsuario' => $this->session->userdata('idUsuario'),
                        'usuario' => $this->session->userdata('usuario'));
       
       $datosVistaDepartamento = array(  
                    'registros' => $registrosConsultaDepartamentos,
                    'nombreDepartamento' => $nombreDepartamento,
                    'conteoTotal' => $conteoTotal,
                    'usuario'=> $usuario
                );
       
       return $datosVistaDepartamento;
   }
}